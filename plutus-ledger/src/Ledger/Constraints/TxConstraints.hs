...
data InputConstraint a = InputConstraint
  { icRedeemer :: a,
    icTxOutRef :: TxOutRef
  }
  deriving (Generic)
...
data OutputConstraint a = OutputConstraint
  { ocDatum :: a,
    ocValue :: Value
  }
  deriving (Generic)
...
{-# INLINEABLE mustPayToOtherScript #-}
...
{-# INLINEABLE mustForgeValue #-}

-- | Create the given value
mustForgeValue :: forall i o. Value -> TxConstraints i o
mustForgeValue = foldMap valueConstraint . (AssocMap.toList . Value.getValue)
  where
    valueConstraint (currencySymbol, mp) =
      let hs = Value.currencyMPSHash currencySymbol
       in foldMap (uncurry (mustForgeCurrency hs)) (AssocMap.toList mp)

{-# INLINEABLE mustForgeCurrency #-}
