...
checkForgingAuthorised tx =
  let forgedCurrencies = V.symbols (txForge tx)
   ...
   in traverse_ (throwError . ForgeWithoutScript) forgedWithoutScript
...
checkMatch txinfo = \case
  ScriptMatch ix vl r d -> do
    let ptx' = ValidatorCtx {valCtxTxInfo = txinfo, valCtxInput = ix}
