...
validatorHash vl = ValidatorHash $ BSL.fromStrict $ BA.convert h'
  where
...
monetaryPolicyHash vl = MonetaryPolicyHash $ BSL.fromStrict $ BA.convert h'
  where
    h :: Digest SHA256 = hash $ BSL.toStrict e
    h' :: Digest SHA256 = hash h
    e = serialise vl
