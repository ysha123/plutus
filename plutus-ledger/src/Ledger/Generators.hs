...
genInitialTransaction GeneratorModel {..} =
  let o = (uncurry $ flip pubKeyTxOut) <$> Map.toList gmInitialBalance
...
genSizedByteString s =
  let range = Range.linear 0 s
   in BSL.fromStrict <$> Gen.bytes range
...
genValue' valueRange = do
  let -- currency symbol is either a validator hash (bytestring of length 32)
...
validateMockchain (Mockchain blck _) tx = either Just (const Nothing) result
  where
    h = 1
