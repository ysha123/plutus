...
txId tx = TxId $ BSL.fromStrict $ BA.convert h'
  where
    h :: Digest SHA256
...
data TxOutType
  = -- | A pay-to-script output with the given datum hash.
...
scriptTxOut' v a ds = TxOut a v tp
  where
...
updateUtxo t unspent = (unspent `Map.difference` lift' (spentOutputs t)) `Map.union` outs
  where
...
addSignature privK tx = tx & signatures . at pubK ?~ sig
  where
