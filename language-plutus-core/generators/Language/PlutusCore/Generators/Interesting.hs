...
naiveFib iv = runQuote $ do
  ...
  i <- freshName "i"
  u <- freshName "u"
  let intS = mkTyBuiltin @Integer ()
      fib =
        LamAbs () i0 intS $
          mkIterApp
            ()
            (mkIterInst () fix [intS, intS])
            [ LamAbs () rec (TyFun () intS intS)
                ...
                $ mkIterApp
                  ...
                  [ mkIterApp
                      ()
                      (builtinNameAsTerm LessThanEqInteger)
...
genApplyAdd2 = do
  ...
  let term =
        mkIterApp
          ()
          (mkIterInst () applyFun [int, TyFun () int int])
          [ builtinNameAsTerm AddInteger,
            i,
            j
          ]
...
genDivideByZeroDrop = do
  ...
  let term =
        mkIterApp
          ()
          (mkIterInst () Function.const [int, int])
...
fromInterestingTermGens ::
  Generatable uni =>
  (forall a. KnownType (Term TyName Name uni ()) a => String -> TermGen uni a -> c) ->
  [c]
