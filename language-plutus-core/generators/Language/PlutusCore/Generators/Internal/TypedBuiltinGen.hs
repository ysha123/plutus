...
data TermOf uni a = TermOf
  { -- | The PLC term
    _termOfTerm :: Term TyName Name uni (),
    -- | The Haskell value.
    _termOfValue :: a
  }
...
instance
  (PrettyBy config a, PrettyBy config (Term TyName Name uni ())) =>
  PrettyBy config (TermOf uni a)
  where
...
updateTypedBuiltinGen ::
  forall a uni m.
  (GShow uni, KnownType (Term TyName Name uni ()) a, Monad m) =>
  -- | A new generator.
  GenT m a ->
