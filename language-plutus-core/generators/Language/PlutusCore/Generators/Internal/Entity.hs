...
newtype BuiltinGensT uni m = BuiltinGensT
  { -- | Generates a PLC 'Term' and the corresponding
    ...
    _builtinGensTyped :: TypedBuiltinGenT uni m
  }
...
data IterAppValue uni head arg r = IterAppValue
  { -- | As a PLC 'Term'.
    _iterTerm :: Term TyName Name uni (),
    -- | As an 'IterApp'.
...
instance
  ( PrettyBy config (Term TyName Name uni ()),
    PrettyBy config head,
    PrettyBy config arg,
    PrettyConst r
  ) =>
  PrettyBy config (IterAppValue uni head arg r)
  where
...
genIterAppValue (Denotation object embed meta scheme) = result
  where
    ...
    go ::
      ...
      FoldArgs args res ->
      PlcGenT uni m (IterAppValue uni head (Term TyName Name uni ()) res)
    go (TypeSchemeResult _) term args y = do
      -- Computed the result.
      let pia = IterApp object $ args []
      return $ IterAppValue term pia y
    go (TypeSchemeArrow _ schB) term args f = do
...
withAnyTermLoose ::
  (Generatable uni, Monad m) =>
  (forall a. KnownType (Plain Term uni) a => TermOf uni a -> GenT m c) ->
  GenT m c
