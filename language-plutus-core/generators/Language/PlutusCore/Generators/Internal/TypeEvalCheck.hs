...
instance
  (PrettyBy config (Error uni ()), PrettyBy config (Plain Term uni)) =>
  PrettyBy config (TypeEvalCheckError uni)
  where
...
typeEvalCheckBy ::
  ( KnownType (Term TyName Name uni ()) a,
    GShow uni,
    GEq uni,
    DefaultUni <: uni,
    Closed uni,
...
typeEvalCheckBy eval (TermOf term x) =
  TermOf term <$> do
...
unsafeTypeEvalCheck ::
  ...
  TermOf uni a ->
  TermOf uni (EvaluationResult (Term TyName Name uni ()))
