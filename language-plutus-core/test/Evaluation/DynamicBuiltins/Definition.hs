...
dynamicFactorialMeaning = DynamicBuiltinNameMeaning sch fac (\_ -> ExBudget 1 1)
  where
...
dynamicConstMeaning = DynamicBuiltinNameMeaning sch Prelude.const (\_ _ -> ExBudget 1 1)
  where
    sch =
...
dynamicIdMeaning = DynamicBuiltinNameMeaning sch Prelude.id (\_ -> ExBudget 1 1)
  where
    sch =
      TypeSchemeAllType @"a" @0 Proxy $ \a ->
  ...
  testCase "dynamicId" $ do
    let env = insertDynamicBuiltinNameDefinition dynamicIdDefinition mempty
        ...
        term =
          mkIterApp
            ()
            (TyInst () dynamicId (TyFun () integer integer))
