...
thunkRecursions =
  testNested "thunkRecursions" $
    map
      (goldenPir ThunkRec.thunkRecursions term)
      [ "listFold",
        "monoMap"
      ]
...
nonStrict =
  testNested "nonStrict" $
    map
      (goldenPir (runQuote . NonStrict.compileNonStrictBindings) term)
      [ "nonStrict1"
      ]
...
letFloat =
  let means = PLC.getStringBuiltinMeanings @(PLC.Term PLC.TyName PLC.Name PLC.DefaultUni ())
   in testNested "letFloat" $
        map
          (goldenPir (LetFloat.floatTerm means . runQuote . PLC.rename) term)
          [ "letInLet",
            "listMatch",
            "maybe",
            "ifError",
