...
runDefT x act = do
  ...
  where
    bindingDefs defs =
      let terms = mapDefs (\d -> TermBind x (snd $ PLC.defVal d) (PLC.defVar d) (fst $ PLC.defVal d)) (_termDefs defs)
...
defSccs tds =
  let perKeyDeps = fmap (\(key, (_, deps)) -> (key, deps)) (Map.assocs tds)
      keySccs = AM.scc (AM.fromAdjacencySets perKeyDeps)
   in -- the graph made by 'scc' is guaranteed to be acyclic
...
defineDatatype ::
  forall key uni ann m.
  MonadDefs key uni ann m =>
  key ->
...
lookupDestructor ::
  ...
  ann ->
  key ->
  m (Maybe (Term TyName Name uni ann))
