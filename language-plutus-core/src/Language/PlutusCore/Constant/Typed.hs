...
instance
  ...
  FromConstant (Term tyname name uni ExMemory)
  where
...
instance
  (KnownTypeAst (UniOf term) a, KnownType term a) =>
  KnownType term (EvaluationResult a)
  where
...
instance
  (KnownSymbol text, KnownNat uniq, UniOf term ~ uni) =>
  KnownTypeAst uni (Opaque term text uniq)
  where
  toTypeAst _ =
    TyVar () . TyName $
      Name
...
instance
  (term ~ term', KnownSymbol text, KnownNat uniq) =>
  KnownType term (Opaque term' text uniq)
  where
