...
dynamicCharToStringMeaning = DynamicBuiltinNameMeaning sch pure (\_ -> ExBudget 1 1)
  where
    sch =
      Proxy @Char
        `TypeSchemeArrow` TypeSchemeResult (Proxy @String)
...
dynamicAppendMeaning ::
  ...
  DynamicBuiltinNameMeaning term -- TODO the costing function
dynamicAppendMeaning = DynamicBuiltinNameMeaning sch (++) (\_ _ -> ExBudget 1 1)
  where
    sch =
      Proxy @String
        `TypeSchemeArrow` Proxy @String
        `TypeSchemeArrow` TypeSchemeResult (Proxy @String)
...
dynamicTraceMeaningMock ::
  ...
  DynamicBuiltinNameMeaning term -- TODO the costing function
dynamicTraceMeaningMock = DynamicBuiltinNameMeaning sch (flip trace ()) (\_ -> ExBudget 1 1)
  where
    sch =
      Proxy @String
        `TypeSchemeArrow` TypeSchemeResult (Proxy @())
