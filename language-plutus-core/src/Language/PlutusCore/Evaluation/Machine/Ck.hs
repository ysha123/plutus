...
data Frame uni
  = -- | @[V _]@
    FrameApplyFun (Plain Value uni)
  | -- | @[_ N]@
    FrameApplyArg (Plain Term uni)
  | -- | @{_ A}@
...
substituteDb ::
  Eq name =>
  name ->
  Value tyname name uni a ->
  Term tyname name uni a ->
  Term tyname name uni a
substituteDb varFor new = go
  where
...
evaluateCk ::
  (GShow uni, GEq uni, DefaultUni <: uni) =>
  Plain Term uni ->
  Either (CkEvaluationException uni) (Plain Term uni)
