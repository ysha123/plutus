...
data Frame uni
  = -- | @[V _]@
    FrameApplyFun (VarEnv uni) (WithMemory Value uni)
  | -- | @[_ N]@
    FrameApplyArg (VarEnv uni) (WithMemory Term uni)
  | -- | @{_ A}@
    FrameTyInstArg (Type TyName uni ExMemory)
  | -- | @(unwrap _)@
    FrameUnwrap
  | -- | @(iwrap A B _)@
    FrameIWrap ExMemory (Type TyName uni ExMemory) (Type TyName uni ExMemory)
  deriving (Show)
...
lookupDynamicBuiltinName dynName = do
  ...
  case Map.lookup dynName means of
    Nothing -> throwingWithCause _MachineError err $ Just term
      where
...
withScopedArgIn ::
  -- | The caller's envorinment.
  VarEnv uni ->
  -- | The argument's environment.
  VarEnv uni ->
  -- | The argument.
  WithMemory Value uni ->
  -- | A continuation handling the argument.
  (WithMemory Value uni -> CekM uni a) ->
  CekM uni a
...
runCek means mode params term =
  runCekM
    (CekEnv means mempty mode params)
