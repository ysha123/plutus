...
typeBinderDocM ::
  ( MonadReader env m,
    HasPrettyConfigReadable env configName,
...
instance
  (PrettyReadableBy configName tyname, GShow uni) =>
...
instance
  ...
  PrettyBy (PrettyConfigReadable configName) (Term tyname name uni a)
  where
  prettyBy = inContextM $ \case
...
instance
  PrettyReadableBy configName (Term tyname name uni a) =>
  PrettyBy (PrettyConfigReadable configName) (Program tyname name uni a)
  where
