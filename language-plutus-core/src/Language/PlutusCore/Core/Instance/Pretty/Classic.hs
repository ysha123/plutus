...
instance
  (PrettyClassicBy configName tyname, GShow uni) =>
  PrettyBy (PrettyConfigClassic configName) (Type tyname uni a)
  where
  prettyBy config = cata a
    where
...
instance
  ...
  PrettyBy (PrettyConfigClassic configName) (Term tyname name uni a)
  where
  prettyBy config = cata a
    where
...
instance
  PrettyClassicBy configName (Term tyname name uni a) =>
  PrettyBy (PrettyConfigClassic configName) (Program tyname name uni a)
  where
