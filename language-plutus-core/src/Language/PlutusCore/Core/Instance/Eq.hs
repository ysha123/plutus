...
instance
  (HasUniques (Term tyname name uni ann), GEq uni, Closed uni, uni `Everywhere` Eq) =>
  Eq (Term tyname name uni ann)
  where
  term1 == term2 = runEqRename $ eqTermM term1 term2

instance
  (HasUniques (Program tyname name uni ann), GEq uni, Closed uni, uni `Everywhere` Eq) =>
  Eq (Program tyname name uni ann)
  where
