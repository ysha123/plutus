...
instance
  (tyname ~ TyName, name ~ Name) =>
  AsNormCheckError (Error uni ann) tyname name uni ann
  where
...
instance
  ...
  PrettyBy config (NormCheckError tyname name uni ann)
  where
...
instance
  (GShow uni, Closed uni, uni `Everywhere` PrettyConst, Pretty ann) =>
  PrettyBy PrettyConfigPlc (TypeError uni ann)
  where
...
instance
  (GShow uni, Closed uni, uni `Everywhere` PrettyConst, Pretty ann) =>
  PrettyBy PrettyConfigPlc (Error uni ann)
  where
