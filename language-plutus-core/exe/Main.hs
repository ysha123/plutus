...
runTypecheck (TypecheckOptions inp fmt) = do
  ...
  case PLC.runQuoteT $ do
    ...
    PLC.typecheckPipeline (PLC.TypeCheckConfig types) (void prog) of
...
toTermExample term = TermExample ty term
  where
