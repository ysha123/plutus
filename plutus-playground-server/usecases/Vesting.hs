...
retrieveFundsC vesting payment = do
  ...
  let currentlyLocked = foldMap (Validation.txOutValue . Tx.txOutTxOut . snd) (Map.toList unspentOutputs)
...
endpoints = vestingContract vestingParams
  where
    ...
    vestingTranche1 =
      VestingTranche
        { vestingTrancheDate = Slot 20,
          vestingTrancheAmount = Ada.lovelaceValueOf 5
        }
    vestingTranche2 =
      VestingTranche
        { vestingTrancheDate = Slot 40,
          vestingTrancheAmount = Ada.lovelaceValueOf 3
        }
