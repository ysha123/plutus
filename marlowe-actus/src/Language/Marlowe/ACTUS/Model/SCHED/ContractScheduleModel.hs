...
_SCHED_PP_PAM scfg _PREF _OPCL _IED _OPANX _MD =
  let maybeS
        | isNothing _OPANX && isNothing _OPCL = Nothing
...
_SCHED_FP_PAM scfg _FER _FECL _IED _FEANX _MD =
  let maybeS
        | isNothing _FEANX && isNothing _FECL = Nothing
...
_SCHED_RR_PAM scfg _IED _SD _RRANX _RRCL _RRNXT _MD =
  let maybeS
      ...
      result
        | isNothing _RRANX && isNothing _RRCL = Nothing
...
_SCHED_SC_PAM scfg _IED _SCEF _SCANX _SCCL _MD =
  let maybeS
        | isNothing _SCANX && isNothing _SCCL = Nothing
