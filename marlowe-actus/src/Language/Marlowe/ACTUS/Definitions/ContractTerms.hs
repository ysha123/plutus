...
data PYTP = PYTP_A | PYTP_N | PYTP_I | PYTP_O deriving (Show, Eq, Generic)
  deriving anyclass (FromJSON, ToJSON)

data FEB = FEB_A | FEB_N deriving (Show, Eq, Generic)
  deriving anyclass (FromJSON, ToJSON)
...
data CalendarType = NoCalendar | MondayToFriday | CustomCalendar {holidays :: [Day]} deriving (Show, Generic)
  deriving anyclass (FromJSON, ToJSON)
...
data ICB = ICB_NT | ICB_NTIED | ICB_NTL deriving (Show, Eq, Generic)
  deriving anyclass (FromJSON, ToJSON)

--FeeBasis
data FB = FB_A | FB_N deriving (Show, Eq, Generic)
  deriving anyclass (FromJSON, ToJSON)

--PenaltyType
data PT = PT_O | PT_A | PT_N | PT_I deriving (Show, Eq, Generic)
  deriving anyclass (FromJSON, ToJSON)

--PrepaymentEffect
data PE = PE_N | PE_A | PE_M deriving (Show, Eq, Generic)
  deriving anyclass (FromJSON, ToJSON)
...
data Stub = ShortStub | LongStub deriving (Show, Eq, Ord, Generic)
  deriving anyclass (FromJSON, ToJSON)
...
data ContractType = PAM | LAM deriving stock (Show, Generic)
  deriving anyclass (FromJSON, ToJSON)
