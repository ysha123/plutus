...
genStaticContract terms =
  let cfs = genProjectedCashflows terms
...
genFsContract terms =
  let payoffAt t = ValueId $ fromString $ "payoff_" ++ show t
      ...
      gen (cf, ev, date, r, t) cont =
        inquiryFs ev ("_" ++ show t) date "oracle" $
          stateTransitionFs ev terms t (cashCalculationDay cf) $
            Let (payoffAt t) (payoffFs ev terms t (cashCalculationDay cf)) $
              if r > 0.0
                then invoice "party" "counterparty" (UseValue $ payoffAt t) date cont
