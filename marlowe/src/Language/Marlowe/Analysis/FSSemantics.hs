...
data SymInput
  = SymDeposit AccountId Party Token SInteger
...
convertToSymbolicTrace [] ((a, b, c, d) : t) =
  (a .== -1) .&& (b .== -1) .&& (c .== -1)
    .&& (d .== -1)
    .&& convertToSymbolicTrace [] t
...
symEvalVal (AddValue lhs rhs) symState =
  symEvalVal lhs symState
    + symEvalVal rhs symState
symEvalVal (SubValue lhs rhs) symState =
  symEvalVal lhs symState
    - symEvalVal rhs symState
symEvalVal (MulValue lhs rhs) symState =
  symEvalVal lhs symState
    * symEvalVal rhs symState
...
symEvalObs (ValueGE lhs rhs) symState =
  symEvalVal lhs symState
    .>= symEvalVal rhs symState
symEvalObs (ValueGT lhs rhs) symState =
  symEvalVal lhs symState
    .> symEvalVal rhs symState
symEvalObs (ValueLT lhs rhs) symState =
  symEvalVal lhs symState
    .< symEvalVal rhs symState
symEvalObs (ValueLE lhs rhs) symState =
  symEvalVal lhs symState
    .<= symEvalVal rhs symState
symEvalObs (ValueEQ lhs rhs) symState =
  symEvalVal lhs symState
    .== symEvalVal rhs symState
...
updateSymInput (Just (SymDeposit accId _ tok val)) symState =
  let resultVal =
        M.findWithDefault 0 (accId, tok) (symAccounts symState)
          + smax (literal 0) val
   in return
        ( symState
            { symAccounts =
...
isValidAndFailsAux oa hasErr (Pay accId payee token val cont) sState =
  do
    ...
    let newAccs =
          M.insert
            (accId, token)
            (smax (literal 0) remainingMoneyInAccount)
            (symAccounts sState)
    let finalSState =
          sState
            { symAccounts =
...
applyInputConditions oa ls hs hasErr maybeSymInput timeout sState pos cont =
  do
    (newCond, newSState) <- addTransaction ls hs maybeSymInput timeout sState pos
...
isValidAndFailsWhen
  oa
  hasErr
  (Case (Deposit accId party token val) cont : rest)
  timeout
  timCont
  previousMatch
  sState
  pos =
    do
      (newLowSlot, newHighSlot, sStateWithInput) <- addFreshSlotsToState sState
      ...
      let newPreviousMatch otherSymInput pmSymState =
            let pmConcVal = symEvalVal val pmSymState
             in case otherSymInput of
...
isValidAndFailsWhen
  oa
  hasErr
  (Case (Choice choId bnds) cont : rest)
  timeout
  timCont
  previousMatch
  sState
...
isValidAndFailsWhen
  ...
  pos =
    do
      ...
      let clashResult = previousMatch symInput sStateWithInput
      let newPreviousMatch otherSymInput pmSymState =
            let pmObsRes = symEvalObs obs pmSymState
             in case otherSymInput of
...
interpretResult t@((l, _, _, _) : _) c maybeState = (Slot l, tin, twa)
  where
    (tin, twa) =
      foldl'
        ( \(accInp, accWarn) (elemInp, elemWarn) ->
