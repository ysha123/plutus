...
{-# INLINEABLE validateInputWitness #-}

{-# INLINEABLE validateInputs #-}
...
data ApplyAllResult
  = ApplyAllSuccess [TransactionWarning] [Payment] State Contract
    ...
    (low, high)
      ...
      | otherwise ->
        let curMinSlot = minSlot state
         ...
         in if high < curMinSlot
              then IntervalError (IntervalInPastError curMinSlot interval)
   ...
   in case value of
        ...
        Scale s rhs ->
          let (n, d) = (numerator s, denominator s)
              nn = eval rhs * n
              (q, r) = nn `quotRem` d
           in if abs r * 2 < abs d then q else q + signum nn * signum d
        ChoiceValue choiceId ->
...
giveMoney payee (Token cur tok) amount accounts = case payee of
  ...
  Account accId ->
    let newAccs = addMoneyToAccount accId (Token cur tok) amount accounts
...
contractLifespanUpperBound contract = case contract of
  ...
  When cases timeout subContract ->
    let contractsLifespans = fmap (\(Case _ cont) -> contractLifespanUpperBound cont) cases
...
validatePayments MarloweParams {..} ctx txOutPayments = all checkValidPayment listOfPayments
  where
    ...
    collectPayments payments (Payment party money) =
      let newValue = money + fromMaybe zero (Map.lookup party payments)
      ...
      case getContinuingOutputs ctx of
        [ TxOut
            ...
            case PlutusTx.fromData ds of
              Just expected ->
                let validContract =
                      txOutState == marloweState expected
...
marloweValidator marloweParams MarloweData {..} inputs ctx@ValidatorCtx {..} =
  let {-  We require Marlowe Tx to have both lower bound and upper bounds in 'SlotRange'.
          All are inclusive.
      -}
  ...
  parseJSON = withArray "ChoiceId" $ \arr ->
    withText
      ...
      ( \name -> do
          ...
          return $ ChoiceId (fromStrict (encodeUtf8 name)) party
      )
      (arr ! 0)

instance ToJSON ChoiceId where
  toJSON (ChoiceId x acc) =
    JSON.Array $
      fromList
  ...
  parseJSON v =
    withObject
      ...
      ( \tk -> do
          ...
          tok <- tk .: "token"
          tok <- parseJSON tok
          return $ Token curr tok
      )
      v

instance ToJSON Token where
  toJSON (Token "" "") = JSON.String "ada"
  toJSON (Token cur tok) =
    JSON.object
      [ ("currency", toJSON cur),
        ("token", toJSON tok)
      ]
...
instance Eq Action where
  ...
  Choice cid1 bounds1 == Choice cid2 bounds2 =
    cid1 == cid2 && length bounds1 == length bounds2
