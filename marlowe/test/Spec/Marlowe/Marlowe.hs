...
zeroCouponBondTest = checkMarloweTrace
  ( MarloweScenario
      { mlInitialBalances =
          Map.fromList
            [(walletPubKey alice, adaValueOf 1000), (walletPubKey bob, adaValueOf 1000)]
      }
  )
  $ do
    -- Init a contract
          ...
          When
            [ Case
                ...
                ( Pay
                    ...
                    (Party bobPk)
                    ada
                    (Constant 850_000_000)
                    ( When
                        [ Case
                            (Deposit aliceAcc bobPk ada (Constant 1000_000_000))
                            ( Pay
                                aliceAcc
                                (Party alicePk)
                                ada
...
zeroCouponBondRolesTest = checkMarloweTrace
  ( MarloweScenario
      { mlInitialBalances =
          Map.fromList
            [ (walletPubKey alice, adaValueOf 1000 <> aliceToken),
              (walletPubKey bob, adaValueOf 1000 <> bobToken)
            ]
      }
  )
  $ do
          ...
          When
            [ Case
                ...
                ( Pay
                    ...
                    ( When
                        [ Case
                            (Deposit aliceAcc bobRole ada (Constant 1000_000_000))
                            ( Pay
                                aliceAcc
                                (Party aliceRole)
                                ada
...
trustFundTest = checkMarloweTrace
  ( MarloweScenario
      { mlInitialBalances =
          Map.fromList
            [(walletPubKey alice, adaValueOf 1000), (walletPubKey bob, adaValueOf 1000)]
  ...
  $ do
    ...
    let contract =
          When
            [ Case
                (Choice chId [Bound 100_000000 1500_000000])
                ( When
                    [ Case
                        (Deposit aliceAcc alicePk ada (ChoiceValue chId))
                        ( When
                            [ Case
                                (Notify (SlotIntervalStart `ValueGE` Constant 150))
                                ( Pay
                                    aliceAcc
                                    (Party bobPk)
                                    ada
...
makeProgressTest = checkMarloweTrace
  ( MarloweScenario
      { mlInitialBalances =
          Map.fromList
            [(walletPubKey alice, adaValueOf 1000), (walletPubKey bob, adaValueOf 1000)]
  ...
  $ do
    ...
    let contract =
          If
            (SlotIntervalStart `ValueLT` Constant 10)
            ( When
                [ Case
                    (Deposit aliceAcc alicePk ada (Constant 500_000000))
                    ( Pay
                        aliceAcc
                        (Party bobPk)
                        ada
                        (AvailableMoney aliceAcc ada)
                        Close
                    )
                ]
                (Slot 100)
  ...
  unsafePerformIO
    ( do
        ...
        res2 <-
            ...
            (wrapLeft $ OldAnalysis.warningsTrace cont)
            (\exc -> return $ Left (Left (exc :: SomeException)))
        return
          ( case (res, res2) of
