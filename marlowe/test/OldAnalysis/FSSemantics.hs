...
convertParty party maps@Mappings {partyM = partyNumberings} = (newParty, newMaps)
  where
    (newParty, newPartyNumberings) = getNumbering party partyNumberings
...
getOriginalOwner numAccId maps = fst $ convertParty ownerParty maps
  where
    (MS.AccountId _ ownerParty, _, _) = revertAccId (AccountId numAccId 0) maps
...
inBounds num =
  foldl'
    (\acc (l, u) -> acc .|| ((num .>= literal l) .&& (num .<= literal u)))
    sFalse
...
emptySState bnds sn =
  ST.tuple
    ( IntegerArray.empty (numAccounts bnds),
...
data Input
  = IDeposit NAccountId Party Money
...
evalValue bnds env state value =
  case value of
    ...
    Scale s rhs ->
      let (n, d) = (numerator s, denominator s)
          nn = go rhs * literal n
          (q, r) = nn `sQuotRem` literal d
       in ite (abs r * 2 .< literal (abs d)) q (q + signum nn * literal (signum d))
    ChoiceValue (ChoiceId c p) ->
      SM.maybe
        (literal 0)
        id
...
giveMoney accs (Party party) mon =
  ST.tuple
    ( sReduceNormalPay (literal party) mon,
      accs
    )
giveMoney accs (Account accId) mon =
  ...
  where
    newAccs = addMoneyToAccount accs (unNestAccountId accId) mon
...
data ReduceResult
  = Reduced NReduceWarning NReduceEffect State
  | NotReduced
...
reduce ::
  ...
  SState ->
  Contract ->
  (SReduceResult -> DetReduceResult -> SBV a) ->
  SBV a
...
reduce bnds env state (Pay accId payee val nc) f =
  ite
    ...
    ( f
        ( sReduced
            ( sReduceNonPositivePay
                (literalAccountId accId)
  ...
  where
    mon = evalValue bnds env state val
    (paidMon, newAccs) =
      ST.untuple $
...
reduce _ env state (When _ timeout c) f =
  ite
    (endSlot .< literal timeout)
    (f sNotReduced DRRNoProgressNormal)
    ( ite
        (startSlot .>= literal timeout)
  ...
  where
    (startSlot, endSlot) = ST.untuple $ slotInterval env
...
data ReduceAllResult
  = ReducedAll [NReduceWarning] [NReduceEffect] State
  | ReduceAllError NReduceError
  deriving (Eq, Show)

mkSymbolicDatatype ''ReduceAllResult

data DetReduceAllResult
  = DRARContractOver
  | DRARError
...
splitReduceResultReduce ::
  Bounds ->
  SList NReduceWarning ->
  SList NReduceEffect ->
  SSReduceResult ->
  ( SList NReduceWarning,
    SList NReduceEffect,
  ...
  where
    contFunc sr dr =
      let (nwa, nef, nsta, err) =
            ST.untuple
              ( symCaseReduceResult
                  (ST.tuple . splitReduceResultReduce bnds wa ef)
                  sr
              )
       in case dr of
            DRRContractOver ->
              f
                (sReducedAll wa ef {- ToDo: extract refunds -} sta)
                DRARContractOver
...
reduceAll ::
  ...
  SState ->
  Contract ->
  (SReduceAllResult -> DetReduceAllResult -> SBV a) ->
  SBV a
...
data TransactionWarning
  = TransactionReduceWarnings [NReduceWarning]
...
splitReduceAllResult ::
  Bounds ->
  SList NTransactionWarning ->
  SList NReduceEffect ->
  SSReduceAllResult ->
  SBV ([NTransactionWarning], [NReduceEffect], State, NReduceError)
splitReduceAllResult bnds wa ef (SSReducedAll twa tef tsta) =
  ST.tuple
    ( ite
        (SL.null twa)
        wa
        (wa SL..++ (sTransactionReduceWarnings twa SL..: [])),
      tef SL..++ ef,
      tsta,
      sReduceAmbiguousSlotInterval {- SNH -}
    )
...
applyCases ::
  ...
  SSInput ->
  [Case] ->
  (SApplyResult -> DetApplyResult -> SBV a) ->
  SBV a
applyCases
  bnds
  env
  state
  inp@(SSIDeposit accId1 party1 mon1)
  (Case (Deposit accId2 party2 val2) nc : t)
  f =
    ite
      ((accId1 .== sAccId2) .&& (party1 .== sParty2) .&& (mon1 .== mon2))
      (f (sApplied warning newState) (DARNormal nc 1))
      (applyCases bnds env state inp t f)
    where
      sAccId2 = literalAccountId accId2
...
applyCases
  ...
  env
  state
  inp@(SSIChoice choId1 cho1)
  (Case (Choice choId2 bounds2) nc : t)
  f =
    ite
      ((choId1 .== sChoId2) .&& inBounds cho1 bounds2)
      (f (sApplied sApplyNoWarning newState) (DARNormal nc 1))
      (applyCases bnds env state inp t f)
    where
      newState =
        state
          `setChoice` (IntegerArray.insert (choiceNumber choId2) cho1 $ choice state)
...
data ApplyAllResult
  = AppliedAll [NTransactionWarning] [NReduceEffect] State
...
applyAllAux ::
  ...
  SList NTransactionWarning ->
  SList NReduceEffect ->
  (SApplyAllResult -> DetApplyAllResult -> SBV a) ->
  SBV a
applyAllAux n bnds numPays numInps env state c l wa ef f
  ...
  where
    contFunReduce sr DRARError =
      let (_, _, _, err) = ST.untuple $ splitReduceAllResultWrap bnds wa ef sr
       in f (sAAApplyError err) DAARError
    contFunReduce sr DRARContractOver =
      let (nwa, nef, nstate, _) = ST.untuple $ splitReduceAllResultWrap bnds wa ef sr
       in ite
            (SL.null l)
      ...
      f
        ( symCaseApplyResult
            ( \case
                ...
                SSApplyError err -> sAAApplyError err
            )
            sr
        )
        DAARError
        ...
        ( \case
            SSApplied twa nst ->
              applyAllAux
                ...
                nc
                t
                (symCaseApplyWarning (addIfEffWa nwa) twa)
                nef
                f
            SSApplyError err -> f (sAAApplyError err) DAARError {- SNH -}
        )
        sr
...
data DetTransactionResult
  = DTProcessed Contract
  | DTError
...
process bnds tra sta c f =
  symCaseIntervalResult
    (\interv -> processAux bnds tra sta c f interv)
...
extractTransactionResult bnds spr =
  ...
  where
    prCases (SSTransactionProcessed wa ef {- ts to -} nst) = ST.tuple (wa, ef {- ts, to, -}, nst)
...
convertAccId accId@(MS.AccountId _ party) cs tok maps =
  ...
  where
    accountNumberings = accountM maps
...
convertPayee (MS.Account accId) cs tok maps = (Account (nestAccountId newAccId), mapsWithAccId)
  where
    (newAccId, mapsWithAccId) = convertAccId accId cs tok maps
convertPayee (MS.Party party) _ _ maps = (Party newParty, mapsWithParty)
  where
    (newParty, mapsWithParty) = convertParty party maps
...
convertValue (MS.ChoiceValue choId) maps =
  ...
  where
    (newChoId, mapsWithChoId) = convertChoId choId maps
...
convertValue (MS.UseValue valId) maps =
  ...
  where
    (newValId, mapsWithValId) = convertValId valId maps
...
convertObservation (MS.AndObs obs1 obs2) maps =
  ...
  where
    (newObs1, mapsWithObs1) = convertObservation obs1 maps
...
convertObservation (MS.NotObs obs) maps =
  ...
  where
    (newObs, mapsWithObs) = convertObservation obs maps
...
convertAction (MS.Notify observation) maps =
  ...
  where
    (newObservation, mapsWithObservation) = convertObservation observation maps
...
convertCaseList (MS.Case action cont : rest) maps =
  ...
  where
    (newAction, mapsWithAction) = convertAction action maps
...
convertContract (MS.When caseList timeout cont) maps =
  ...
  where
    (newCaseList, actionsWithCaseList, mapsWithCaseList) = convertCaseList caseList maps
...
extractBounds maxActions maps =
  Bounds
    { numParties = numberOfLabels (partyM maps),
...
revertTransactionList list maps =
  [ MS.TransactionInput
      { MS.txInterval = revertInterval slotInter,
...
revertReduceWarningList (ReduceShadowing valId int1 int2) maps =
  ...
  where
    newValId = revertValId (unNestValueId valId) maps
...
revertTransactionWarningList (TransactionApplyWarning applyWarning) maps =
  case unNestApplyWarning applyWarning of
    (ApplyNonPositiveDeposit party accId amount) ->
      let (cAccId, cs, tok) = revertAccId (unNestAccountId accId) maps
       in [ MS.TransactionNonPositiveDeposit
              (revertParty party maps)
              cAccId
              (MS.Token cs tok)
              amount
          ]
...
extractCounterExample smtModel maps func =
  ( Slot slotNum,
    revertTransactionList transList maps,
    revertAllWarnings
  )
  where
    assocs = modelAssocs smtModel
    ...
    revertAllWarnings =
      concatMap
        ( \x ->
            revertTransactionWarningList
              (unNestTransactionWarning x)
              maps
        )
        warningList
...
transSlotsPositive trans = slotMin .>= 0 .&& slotMax .>= 0
  where
    slotInt = interval trans :: SSlotInterval
