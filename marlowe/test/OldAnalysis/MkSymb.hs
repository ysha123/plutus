...
mkSubSymDec tvs ssName clauses =
  dataD
    ...
    Nothing
    [ normalC
        ( let typeBaseName = nameBase name
           in mkName ('S' : 'S' : typeBaseName)
        )
        [ bangType
            (return pb)
            [t|SBV $(return param)|]
...
nestFunClauses [NormalC name params] f =
  [ do
      names <- mapM (\x -> newName ('p' : show x)) [1 .. (length params)]
      clause
        [conP name [varP n | n <- names]]
        (normalB (f (tupE (map varE names))))
        []
  ]
nestFunClauses list f =
  nestFunClauses fHalf (f . (\x -> [e|Left $(x)|]))
...
mkNestFunc tvs typeBaseName typeName typeNName clauses =
  do
    let nestFunName = "nest" ++ typeBaseName
    ...
    signature <-
      sigD
        ...
        ( addSymValToTVs
            tvs
            ( appT
                (appT arrowT (addTVs tvs $ conT typeName))
...
unNestFunClauses [NormalC name params] f =
  [ do
      names <- mapM (\x -> newName ('p' : show x)) [1 .. (length params)]
      clause
        [f (tupP (map varP names))]
        (normalB (foldl' appE (conE name) [varE n | n <- names]))
        []
  ]
unNestFunClauses list f =
  unNestFunClauses fHalf (f . (\x -> [p|Left $(x)|]))
...
mkUnNestFunc tvs typeBaseName typeName typeNName clauses =
  do
    let unNestFunName = "unNest" ++ typeBaseName
    let unfName = mkName unNestFunName
    signature <-
      sigD
        unfName
        ( appT
            (appT arrowT (addTVs tvs $ conT typeNName))
...
mkSymCaseRec func [NormalC name params] =
  do
    paramNames <- mapM (\x -> newName ('p' : show x)) [1 .. (length params)]
    let ssName = mkName ('S' : 'S' : nameBase name)
    let wrapping =
          [e|
            ( \ $(tupP $ map varP paramNames) ->
                $(varE func)
...
mkSymCaseRec func list =
  [e|
    SE.either
      $(mkSymCaseRec func fHalf)
...
mkSymCase tvs typeBaseName sName ssName clauses =
  do
    let symCaseFunName = "symCase" ++ typeBaseName
...
mkSymConsts tvs sName [NormalC name params] f =
  do
    let nestFunName = "s" ++ nameBase name
    let nfName = mkName nestFunName
    signature <-
      sigD nfName $
        addSymValToTVs
...
mkSymConsts tvs sName list f =
  do
    rfHalf <- mkSymConsts tvs sName fHalf (f . (\x -> [e|sLeft $(x)|]))
...
mkSymbolicDatatype typeName =
  do
    TyConI (DataD _ _ tvs _ clauses _) <- reify typeName
