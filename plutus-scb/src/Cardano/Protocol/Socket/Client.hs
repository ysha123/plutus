...
data ClientConfig m = ClientConfig
  { -- | Path of the socket used to communicate with the server.
    ccSocketPath :: FilePath,
    -- | Client application state.
    ccAppState :: MVar AppState,
    -- | Handler used to store the incoming blocks.
    ccBlockHandler :: Block -> m (),
    -- | A queue used to send new transactions to the server.
    ccTxQueue :: TQueue Tx
  }
