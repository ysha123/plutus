...
instance
  Pretty state =>
  Pretty (StreamProjection key position state event)
  where
...
newtype ContractIterationState = ContractIterationState
...
data IteratedContractState a = IteratedContractState
...
installedContractsProjection ::
  forall t key position.
  Ord t =>
  Projection (Set t) (StreamEvent key position (ChainEvent t))
