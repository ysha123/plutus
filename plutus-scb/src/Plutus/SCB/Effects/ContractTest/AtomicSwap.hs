...
data AtomicSwapParams = AtomicSwapParams
  { -- | The amount paid to the hash of 'party1'
    ada :: Value,
    currencyHash :: CurrencySymbol,
    tokenName :: TokenName,
    amount :: Integer,
    -- | The first party in the atomic swap
    party1 :: Wallet,
    -- | The second party in the atomic swap
    party2 :: Wallet,
    -- | Last slot in which the swap can be executed.
    deadline :: Slot
  }
...
data AtomicSwapError
  ...
  | -- | When the wallet's public key doesn't match either of the two keys specified in the 'AtomicSwapParams'
    NotInvolvedError PubKey AtomicSwapParams
  deriving (Show)
