...
data CustomDefaults = CustomDefaults
  { -- | At what base to pretty-print an 'Integer'.
    _noDefaultsIntBase :: Integer,
    -- | Whether to pretty-print a list using 'prettyListBy'
    -- (of the 'PrettyBy' class) or as any 'Functor' via
    -- 'defaultPrettyFunctorBy'.
    _noDefaultsListDef :: Bool
  }
...
makeTestCase ::
  ...
  String ->
  TestTree
makeTestCase name config x res = testCase header $ show (prettyBy config x) @?= res
  where
...
test_nonDefault =
  testGroup
    ...
    [ makeTestCase "" (CustomDefaults 2 True) 'a' "a",
      ...
      makeTestCase
        "listDef = True "
        (CustomDefaults 2 True)
        (Just ("ab12" :: String))
        "Just (ab12)",
      makeTestCase
        "listDef = False"
        (CustomDefaults 2 False)
        (Just ("ab12" :: String))
        "Just ([a, b, 1, 2])",
