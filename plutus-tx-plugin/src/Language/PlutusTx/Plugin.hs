...
compileMarkedExprs opts markerName =
  let comp = compileMarkedExprs opts markerName
...
compileCoreExpr (opts, famEnvs) locStr codeTy origE = do
  ...
  res <-
    runExceptT . runQuoteT . flip evalStateT initialState . flip runReaderT context $
  ...
  case res of
    Left s ->
      let shown = show $ PP.pretty (pruneContext (poContextLevel opts) s)
       in -- TODO: is this the right way to do either of these things?
          if poDeferErrors opts
            then -- this will blow up at runtime
            do
            ...
            else -- this will actually terminate compilation
              failCompilation shown
    Right (pirP, plcP) -> do
...
runCompiler opts expr = do
  let (ctx :: PIR.CompilationCtx uni ()) =
        PIR.defaultCompilationCtx
