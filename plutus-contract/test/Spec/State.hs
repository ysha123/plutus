    ...
    [ HUnit.testCase "run a contract without prompts" $
      ...
      HUnit.testCase "go into a branch" $
        let record = Responses $ Map.fromList [((IterationID 1, RequestID 2), 5), ((IterationID 2, RequestID 4), 10)]
            (result, _) =
              runResumableTest @Int @String
                record
                ( (askStr "prompt1" >> askStr "prompt4")
                    `selectStr` (askStr "prompt2" >> (askStr "prompt5" `selectStr` (askStr "prompt6" >> pure 11) `selectStr` askStr "prompt8"))
      ...
      HUnit.testCase "loop" $
        let record =
            ...
            (result, _) =
              runResumableTest @Int @String record $
