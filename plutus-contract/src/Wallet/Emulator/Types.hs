...
walletRecvBlocks w nots = void $ walletControlAction w (traverse_ go nots)
  where
...
runTraceChain ch t = runState (runExceptT $ processEmulated t) emState
  where
...
runTraceTxPool tp t = runState (runExceptT $ processEmulated t) emState
  where
