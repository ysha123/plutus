...
emptyWalletState w = WalletState pk
  where
    pk = walletPrivKey w

data PaymentArgs = PaymentArgs
...
handleUpdatePaymentWithChange
  ...
  Payment {paymentInputs = oldIns, paymentChangeOutput = changeOut} = do
    ...
    if requestedValue `Value.leq` oldChange
      then -- If the requested value is covered by the change we only need to update
...
selectCoin fnds vl =
  let total = foldMap snd fnds
   ...
   in -- Values are in a partial order: what we want to check is that the
      ...
      if not (total `Value.geq` vl)
        then err
        else
          let fundsToSpend = takeUntil (\(_, runningTotal) -> vl `Value.leq` runningTotal) fundsWithTotal
