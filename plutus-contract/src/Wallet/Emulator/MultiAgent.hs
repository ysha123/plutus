...
data Assertion
  ...
  | -- | Assert that the funds belonging to a wallet's public-key address are equal to a value.
    OwnFundsEqual Wallet.Wallet Value
...
data EmulatorState = EmulatorState
  { _chainState :: Chain.ChainState,
    -- | The state of each wallet.
    _walletStates :: Map Wallet.Wallet Wallet.WalletState,
    -- | The state of each wallet's node client.
    _walletClientStates :: Map Wallet.Wallet NC.NodeClientState,
    -- | The state of each wallet's chain index
    _walletChainIndexStates :: Map Wallet.Wallet ChainIndex.ChainIndexState,
    -- | The wallet's signing process
    _walletSigningProcessStates :: Map Wallet.Wallet SP.SigningProcess,
...
emulatorStateInitialDist mp = emulatorStatePool [tx]
  where
    tx =
      Tx
        { txInputs = mempty,
...
handleMultiAgent = interpret $ \case
  ...
  WalletControlAction wallet act ->
    act
      & raiseEnd5
