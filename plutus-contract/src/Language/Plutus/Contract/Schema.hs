...
instance (Forall (Input s) Pretty) => Pretty (Event s) where
  pretty (Event e) =
    let (lbl, vl) = Variants.eraseWithLabels @Pretty pretty e
     in hang 1 (braces $ vsep [lbl <> colon, vl])
...
instance (Forall (Output s) Pretty) => Pretty (Handlers s) where
  pretty (Handlers s) =
    let (lbl, vl) = Variants.eraseWithLabels @Pretty pretty s
     in hang 1 (braces $ vsep [lbl <> colon, vl])
...
type family InputR (r :: [LT *]) where
  ...
  InputR (l ':-> t ': _) =
    TypeError
      ( 'Text "Input requires all types to be tuples."
...
type family OutputR (r :: [LT *]) where
  ...
  OutputR (l ':-> t ': _) =
    TypeError
      ( 'Text "Output requires all types to be tuples."
          :$$: 'Text "For one, the field labelled " :<>: ShowType l :<>: 'Text " has type " :<>: ShowType t
      )
