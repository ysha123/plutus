...
import Language.Plutus.Contract.Trace.RequestHandler
  ( MaxIterations (..),
    RequestHandler (..),
    defaultMaxIterations,
    maybeToHandler,
...
data WalletState s e a = WalletState
...
data ContractTraceState s e a = ContractTraceState
...
initState wllts con = ContractTraceState wallets con
  where
...
addEvent wallet event = do
  ...
  hks <-
    mapMaybe filterReq <$> getHooks wallet >>= \case
...
data ContractTraceResult s e a = ContractTraceResult
...
execTrace con action =
  let (e, _) = runTrace con action
   in either (const Map.empty) (fmap (toList . walletEvents) . _ctsWalletStates . snd) e
...
runTraceWithDistribution dist con action =
  let con' = mapError TContractError con
   in withInitialDistribution dist (runStateT action (initState (Map.keys dist) con'))
...
addBlocks' ::
  ...
  -- | Whether to notify all wallets after each block.
  SlotNotifications ->
...
data HandleBlockchainEventsOptions = HandleBlockchainEventsOptions
...
handleBlockchainEventsOptions ::
  ...
  Wallet ->
  ContractTrace s e m a ()
handleBlockchainEventsOptions o wallet = go 0
  where
    ...
    go j
      | j >= i = lift (throwError (HandleBlockchainEventsMaxIterationsExceeded wallet (MaxIterations i)))
      | otherwise = do
