...
import Language.Plutus.Contract.Checkpoint
  ( AsCheckpointError,
    Checkpoint (..),
    CheckpointError (..),
    CheckpointKey,
...
mapError ::
  ...
  Contract s e a ->
  Contract s e' a
mapError f (Contract c) = Contract c'
  where
...
runResumable events store action =
  let record = foldr insertResponse mempty events
   in runWithRecord action store record
...
data ResumableResult i o a = ResumableResult
