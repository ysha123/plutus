...
endpoint ::
  ...
  Contract s e a
endpoint = unEndpointValue <$> request @l @_ @_ @s s
  where
    s =
      ActiveEndpoint
...
endpointWithMeta ::
  ...
  b ->
  Contract s e a
endpointWithMeta b = unEndpointValue <$> request @l @_ @_ @s s
  where
    s =
      ActiveEndpoint
        { aeDescription = EndpointDescription $ symbolVal (Proxy @l),
...
event ::
  forall (l :: Symbol) a s.
  (KnownSymbol l, HasType l (EndpointValue a) (Input s), AllUniqueLabels (Input s)) =>
  a ->
  Event s
...
isActive ::
  forall (l :: Symbol) s.
  (KnownSymbol l, HasType l ActiveEndpoint (Output s)) =>
  Handlers s ->
  Bool
