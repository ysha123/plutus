...
submitUnbalancedTx t =
  let req = request @TxSymbol @_ @_ @s t
   in req >>= either (throwError . review _WalletError) pure . view writeTxResponse
...
event ::
  forall s.
  (HasType TxSymbol WriteTxResponse (Input s), AllUniqueLabels (Input s)) =>
  WriteTxResponse ->
  Event s
