...
class ToSchema a where
  toSchema :: FormSchema
  default toSchema ::
    (Generic a, GenericToSchema (Rep a)) =>
    FormSchema
...
class
  ...
  default toArgument ::
    (Generic a, GenericToArgument (Rep a)) =>
    a ->
    Fix FormArgumentF
...
instance
  (GenericToConstructorName c1, GenericToConstructorName c2) =>
  GenericToSchema (c1 :+: c2)
  where
  genericToSchema _ =
    FormSchemaRadio $
...
instance
  (GenericToConstructorName c1, GenericToConstructorName c2) =>
  GenericToConstructorName (c1 :+: c2)
  where
