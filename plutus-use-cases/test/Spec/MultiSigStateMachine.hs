...
tests =
  testGroup
    ...
    [ checkPredicate @MultiSigSchema @MultiSigError
        "lock, propose, sign 3x, pay - SUCCESS"
        (MS.contract params)
      ...
      checkPredicate @MultiSigSchema @MultiSigError
        "lock, propose, sign 2x, pay - FAILURE"
        (MS.contract params)
      ...
      checkPredicate @MultiSigSchema @MultiSigError
        "lock, propose, sign 3x, pay x2 - SUCCESS"
        (MS.contract params)
        ( assertNoFailedTransactions
            /\ walletFundsChange w1 (Ada.lovelaceValueOf (-10))
      ...
      checkPredicate @MultiSigSchema @MultiSigError
        "lock, propose, sign 3x, pay x3 - FAILURE"
        (MS.contract params)
