...
tests =
  testGroup
    "crowdfunding"
    [ checkPredicate
        "Expose 'contribute' and 'scheduleCollection' endpoints"
        theContract
      ...
      checkPredicate
        "make contributions and collect"
        theContract
        (walletFundsChange w1 (1 `timesFeeAdjust` 21))
        $ successfulCampaign,
      checkPredicate
        "cannot collect money too early"
        theContract
      ...
      checkPredicate
        ...
        $ startCampaign
          ...
          >> Trace.addBlocks 1,
      checkPredicate
        "cannot collect unless notified"
        theContract
        (walletFundsChange w1 PlutusTx.zero)
        $ startCampaign
          >> makeContribution w2 (Ada.lovelaceValueOf 10)
