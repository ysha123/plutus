...
import Language.PlutusTx.Coordination.Contracts.Future
  ( Future (..),
    FutureAccounts (..),
    FutureError,
    FutureSchema,
...
tests =
  testGroup
    ...
    [ checkPredicate @FutureSchema
      ...
      checkPredicate @FutureSchema
        "can initialise and obtain tokens"
...
payOut = do
  let spotPrice = Ada.lovelaceValueOf 1124
...
oracleKeys =
  let wllt = Wallet 10
   in (walletPrivKey wllt, walletPubKey wllt)
...
settleEarly = do
  let spotPrice = Ada.lovelaceValueOf 11240
