...
tests =
  testGroup
    "token account"
    [ checkPredicate @TokenAccountSchema @TokenAccountError
...
account :: Account
account =
  let con = Accounts.newAccount tokenName (Ledger.pubKeyHash $ walletPubKey w1)
   in either error id $ evalTrace @TokenAccountSchema @TokenAccountError con (handleBlockchainEvents w1 >> addBlocks 1 >> handleBlockchainEvents w1 >> addBlocks 1 >> handleBlockchainEvents w1) w1
