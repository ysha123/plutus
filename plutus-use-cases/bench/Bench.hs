...
sumB =
  ...
  where
    ...
    foldrScott f z = go
      where
        go l = Scott.matchList l z (\h t -> f h (go t))

    foldrRecScott :: (a -> b -> b) -> b -> Scott.ScottList a -> b
    foldrRecScott f z = go
      where
        go = Rec.fix1zSimple $ \r l -> Scott.matchList l z (\h t -> f h (r t))
...
scriptHashes =
  bgroup
    "script hashes"
    [ let si = TA.scriptInstance (TA.Account ("fd2c8c0705d3ca1e7b1aeaa4da85dfe5ac6dde64da9d241011d84c0ee97aac5e", "my token"))
       in bench "token account" $ nf Scripts.validatorScript si,
...
accounts =
  let cur = "fd2c8c0705d3ca1e7b1aeaa4da85dfe5ac6dde64da9d241011d84c0ee97aac5e"
   in FT.mkAccounts
        (TA.Account (cur, "long"))
