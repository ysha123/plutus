...
tokenAccountContract ::
  ...
  Contract s e ()
tokenAccountContract = mapError (review _TokenAccountError) (redeem_ `select` pay_ `select` newAccount_)
  where
    redeem_ = do
  ...
  let wrap = Scripts.wrapValidator @() @()
      val =
        $$(PlutusTx.compile [||validate||])
          `PlutusTx.applyCode` PlutusTx.liftCode account
...
redeemTx account pk = mapError (review _TAContractError) $ do
  ...
  let tx =
        TypedTx.collectFromScript utxos ()
