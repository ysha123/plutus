...
data Swap = Swap
  { swapNotionalAmt :: !Ada,
    ...
    swapMargin :: !Ada,
    -- | Public key of the oracle (see note [Oracles] in [[Language.PlutusTx.Coordination.Contracts]])
    swapOracle :: !PubKey
  }
...
mkValidator Swap {..} SwapOwners {..} redeemer p@ValidatorCtx {valCtxTxInfo = txInfo} =
  let extractVerifyAt :: SignedMessage (Observation Rational) -> PubKey -> Slot -> Rational
...
swapValidator swp =
  Ledger.mkValidatorScript $
    $$(PlutusTx.compile [||validatorParam||])
      `PlutusTx.applyCode` PlutusTx.liftCode swp
  where
