...
{-# INLINEABLE futureStateMachine #-}
futureStateMachine ::
  Future ->
  FutureAccounts ->
  StateMachine FutureState FutureAction
futureStateMachine ft fos = SM.mkStateMachine (transition ft fos) isFinal
  where
...
scriptInstance future ftos =
  let val =
        $$(PlutusTx.compile [||validatorParam||])
          `PlutusTx.applyCode` PlutusTx.liftCode future
          `PlutusTx.applyCode` PlutusTx.liftCode ftos
...
transition future@Future {ftDeliveryDate, ftPriceOracle} owners State {stateData = s, stateValue = currentValue} i =
  case (s, i) of
    ...
    (Running accounts, Settle ov)
      | Just (Observation {obsValue = spotPrice, obsSlot = oracleDate}, oracleConstraints) <- verifyOracle ftPriceOracle ov,
        ftDeliveryDate == oracleDate ->
    ...
    (Running accounts, SettleEarly ov)
      | Just (Observation {obsValue = spotPrice, obsSlot = oracleDate}, oracleConstraints) <- verifyOracle ftPriceOracle ov,
        Just vRole <- violatingRole future accounts spotPrice,
        ftDeliveryDate > oracleDate ->
        let total = totalMargin accounts
...
initialState ft =
  let im = initialMargin ft
   in Running (Margins {ftsShortMargin = im, ftsLongMargin = im})

futureAddress :: Future -> FutureAccounts -> Address
futureAddress ft fo = Ledger.scriptAddress (validator ft fo)

{-# INLINEABLE violatingRole #-}
...
violatingRole future margins spotPrice =
  let minMargin = requiredMargin future spotPrice
      Margins {ftsShortMargin, ftsLongMargin} = margins
   in if ftsShortMargin `lt` minMargin
        then Just Short
        else
          if ftsLongMargin `lt` minMargin
            then Just Long
            else Nothing
...
initialiseFuture future = mapError (review _FutureError) $ do
  ...
  let client = machineClient inst future ftos
...
escrowParams client future ftos FutureSetup {longPK, shortPK, contractStart} =
  let address = Ledger.validatorHash $ Scripts.validatorScript $ SM.validatorInstance $ SM.scInstance client
      dataScript = Ledger.Datum $ PlutusTx.toData $ initialState future
      targets =
        [ Escrow.payToScriptTarget
            address
            dataScript
