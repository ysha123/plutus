...
import Marlowe.Language.Marlowe.Client
  ( commit,
    commit',
    interpretObs,
    marloweValidator,
...
checkFindAndRemove = do
  ...
  let commits =
        [ (IdentCC 1, (pk, NotRedeemed 12 10)),
...
notEnoughMoney = do
  let commits =
        [ (IdentCC 1, (pubKey1, NotRedeemed 60 100)),
...
oraclePayment = checkMarloweTrace
  ( MarloweScenario
      { mlInitialBalances = Map.fromList [(pubKey1, Ada.adaValueOf 1000), (pubKey2, Ada.adaValueOf 777)]
      }
  )
  $ do
    -- Init a contract
    ...
    let contract =
          CommitCash
            (IdentCC 1)
            bobPk
            (ValueFromOracle oraclePk (Value 0))
            128
            256
...
cantCommitAfterStartTimeout = checkMarloweTrace
  ( MarloweScenario
      { mlInitialBalances = Map.fromList [(pubKey1, Ada.adaValueOf 1000), (pubKey2, Ada.adaValueOf 777)]
      }
  )
...
redeemAfterCommitExpired = checkMarloweTrace
  ( MarloweScenario
      { mlInitialBalances = Map.fromList [(pubKey1, Ada.adaValueOf 1000), (pubKey2, Ada.adaValueOf 777)]
      }
  )
...
escrowTest :: Property
escrowTest = checkMarloweTrace
  ( MarloweScenario
      { mlInitialBalances = Map.fromList [(pubKey1, Ada.adaValueOf 1000), (pubKey2, Ada.adaValueOf 777), (pubKey3, Ada.adaValueOf 555)]
      }
  )
  $ do
    -- Init a contract
      ...
      tx <-
        alice
          `performs` commit
            ...
            ( When
                ( OrObs
                    (twoChose aliceId alicePk bobId bobPk carolId carolPk 0)
...
futuresTest = checkMarloweTrace
  ( MarloweScenario
      { mlInitialBalances = Map.fromList [(alicePk, Ada.adaValueOf 1000000), (bobPk, Ada.adaValueOf 1000000)]
      }
  )
  $ do
    -- Init a contract
          ...
          CommitCash
            ...
            ( CommitCash
                ...
                ( When
                    ...
                    ( Choice
                        ( AndObs
                            (ValueGE spotPriceV forwardPriceV)
                        ...
                        ( Choice
                            ...
                            ( Pay
                                ...
                                bobPk
                                alicePk
                                (delta (minus spotPriceV forwardPriceV))
                                endTimeout
                                redeems
                            )
                            ( Choice
                                ...
                                ( Pay
                                    ...
                                    alicePk
                                    bobPk
                                    (delta (minus forwardPriceV spotPriceV))
                                    endTimeout
                                    redeems
                                )
                                redeems
      ...
      tx <-
        bob
          `performs` commit
            ...
            initialMargin
            ( State
                [ (IdentCC 1, (alicePk, NotRedeemed initialMargin endTimeout)),
