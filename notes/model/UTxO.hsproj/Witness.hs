...
module Witness
  ( -- ** Scripts
    ...
    script,
    scriptHash,
    revealPreimageValidator,
    lockWithPublicKeyValidator,
    lockWithMultiSigValidator,
    lockWithPublicKeyHashValidator,
  ...
  script
    ...
    \state sigs ->
      let disjoint [] = True
       ...
       in length sigs == requiredSigCount
            && disjoint sigs
            && all
              ( \sig ->
...
witness ::
  Q (TExp (Script (State -> proof -> Bool))) ->
