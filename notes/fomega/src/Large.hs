...
alfa3 =
  [declExp| def = (1 * (list varDecl) * exp * exp)
                      + (1 * exp)
...
alfa10 =
  [declExp| branch = (1 * (list 1) * exp)
                          + (1 * 1 * 1 * exp)
                          + (1 * exp) |]

alfa11 =
  [declExp| constructor = (1 * (list typing))
                               + 1 |]
...
alfa15 =
  [declExp| openArg = ((list defAttr) * 1)
                           + ((list defAttr) * 1 * exp)
                           + ((list defAttr) * 1 * exp)
...
bnfc4 =
  [declExp| label = 1 + (1 * (list profItem))
                        + (1 * 1 * (list profItem))
                        + (1 * 1)|]
...
java1 =
  [declExp| programFile = ((list 1) * (list 1)
                              * (list import) * (list typeDecl))
                              + ((list import) * (list typeDecl))|]

java2 =
  [declExp| import = ((list ident) * (list 1))
                         + ((list ident) * (list 1))|]

java3 = [declExp| typeDecl = classHeader * (list fileDeclaration)|]

java4 =
  [declExp| classHeader = ((list modifier) * 1)
...
java13 =
  [declExp| arrayInits = variableInits
                              + (arrayInits * variableinits)
                              + arrayInits |]

java14 =
  [declExp| methodDecl = (declaratorName * (list parameter))
                              + (methodDecl * bracketsOpt)|]

java15 =
  [declExp| parameter = (typeSpec * declaratorName)
                             + (typeSpec * declaratorName) |]

java16 =
  [declExp| selectionStm = (exp * stm * (list elseIf))
...
java18 =
  [declExp| jumpStm = 1 + (1 * 1) + 1 + (1 * 1) + 1
                           + (1 * exp) + (1 * exp)|]

java19 =
  [declExp| guardStm = (exp * body)
                            + (body * (list catch))
                            + (body * (list catch) * body)|]

java20 =
  [declExp| catch = (typeSpec * 1 * body)
                         + (typeSpec * body)|]

java21 =
  [declExp| iterStm = (exp * stm)
                           + (stm * exp)
...
java24 =
  [declExp| typeSpec = (typeName * (list bracketsOps))
                            + typeName |]
...
java28 =
  [declExp| newAlloc = (typeName * args)
...
java32 =
  [declExp| mthCall = ((list 1) * args)
                           + (specExpNp * args)
                           + (1 * args)|]

java33 =
  [declExp| fieldAcc = (specExp * 1)
                            + (newAlloc * 1)
                            + (list 1) + (list 1)
                            + basicType|]
