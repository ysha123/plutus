...
data Frame
  = -- | @[_ N]@       -- Tells the machine that we've got back to N after evaluating M in [M N]
    FrameAppArg Closure
  | -- | @(update l)@  -- Mark the stack for evaluation of a delayed argument
    FrameHeapUpdate HeapLoc
  | -- | @{_ A}@
    FrameTyInstArg (Type TyName ())
  | -- | @(unwrap _)@
    FrameUnwrap
  | -- | @(wrap α A _)@
...
instantiateEvaluate ctx heap ty (Closure fun env) =
  case fun of
    ...
    _ ->
      if isJust $ termAsPrimIterApp fun
